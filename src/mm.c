...
{                                         // no guarantee all given pages are mapped
  ...
  // Assuming it's mapped to the current VMA
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
  ...
  if(mm->pgd == NULL || vma0 == NULL || vma1 == NULL){
  ...
  vma1->vm_id = 1;
  vma1->vm_start = vma0->vm_end + 1; // Heap starts after DATA
  vma1->vm_end = vma1->vm_start; // Initially empty
